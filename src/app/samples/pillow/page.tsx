...
export default function PillowSamplePage() {
  ...
  const handleSubmit = async (values: SampleFormValues) => {
    ...
    try {
      await saveSample(firestore, user.uid, values, 'Almohada');
      toast({
        title: "Muestra registrada",
        description: "La muestra de Almohada ha sido registrada exitosamente.",
      });
      form.reset();
    } catch(e) {
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
        <Card>
          <CardHeader>
            <CardTitle>Nueva Muestra de Almohada</CardTitle>
            <CardDescription>
              Complete el formulario para registrar una nueva muestra de almohada.
            </CardDescription>
          </CardHeader>
          <CardContent>
